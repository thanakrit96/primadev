...
const aboutComponents = ({ titleAbout, subTitle, data, whyprima, serviceprima, bannerList, kickeraboutClinic, btngroup }) => {
  return (
    <ContainerT>
      {/* section1 */}
      <div style={{ borderRadius: "12px", boxShadow: "2px 2px 8px 4px rgba(0, 0, 0, 0.1)", backgroundColor: "#FFFF", padding: "", zIndex: "-3" }}>
        <div>
      ...
      <div style={{ borderRadius: "12px", boxShadow: "2px 2px 8px 4px rgba(0, 0, 0, 0.1)", backgroundColor: "#FFFF", padding: "" }}>
        {serviceprima.length > 0 && (
          <Title style={{ paddingTop: "2vw" }}>{serviceprima[0].titleserviceprima}</Title>
        )}
        <Conblog>
          {serviceprima.map((serviceprima, i) => (
            <Blog key={i}>
              ...
              {/* <BlurredBackground imageUrl={serviceprima.serpic.url} /> */}
            </Blog>
          ))}
        </Conblog>
      </div>
